    ...
    <AbsoluteFill style={{ backgroundColor: '#050d1a', overflow: 'hidden' }}>
      ...
      <AbsoluteFill style={{ display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', padding: '0 80px 80px' }}>
        ...
        <div style={{ opacity: nameOp, transform: `scale(${nameSc})`, transformOrigin: 'left bottom' }}>
          <div style={{ fontSize: 84, fontWeight: '800', color: '#f1f5f9', fontFamily: DISPLAY_FONT, letterSpacing: '2px', textTransform: 'uppercase' as const, lineHeight: 1 }}>
            {productName}
          </div>
        </div>

        {/* Accent line */}
        <div style={{ width: 60, height: 3, background: accentColor, margin: '16px 0', borderRadius: 2 }} />

        {/* Tagline */}
        <div style={{ opacity: tagOp, transform: `translateY(${tagY}px)` }}>
          <div style={{ fontSize: 30, color: 'rgba(241,245,249,0.85)', fontFamily: FONT, fontWeight: '400', lineHeight: 1.4 }}>
            {tagline}
          </div>
        </div>
        ...
        {/* Price badge */}
        {price && (
          <div style={{ opacity: priceOp, transform: `scale(${priceSc})`, transformOrigin: 'left bottom', marginTop: 24 }}>
            <div style={{
              display: 'inline-flex', alignItems: 'center',
              background: accentColor, borderRadius: 100, padding: '12px 32px',
              boxShadow: `0 0 32px ${av.glow}`,
            }}>
              <span style={{ fontSize: 28, color: '#ffffff', fontFamily: FONT, fontWeight: '700' }}>{price}</span>
            </div>
          </div>
        )}
      </AbsoluteFill>
